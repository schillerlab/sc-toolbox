...
def sc_toolbox_questionary(function: str, question: str, choices: list = None, default: str = None) -> Union[str, bool]:
    """
    ...
    try:
        if function == "select":
            ...
            answer = getattr(questionary, function)(
                f"{question}: ", choices=choices, style=sc_toolbox_style
            ).unsafe_ask()
        elif function == "password":
            while not answer or answer == "":
                answer = getattr(questionary, function)(f"{question}: ", style=sc_toolbox_style).unsafe_ask()
        elif function == "text":
            if not default:
                log.debug(
                ...
                default = ""
            answer = getattr(questionary, function)(f"{question} [{default}]: ", style=sc_toolbox_style).unsafe_ask()
        elif function == "confirm":
            default_value_bool = True if default == "Yes" or default == "yes" else False
            answer = getattr(questionary, function)(
                f"{question} [{default}]: ", style=sc_toolbox_style, default=default_value_bool
            ).unsafe_ask()
        else:
